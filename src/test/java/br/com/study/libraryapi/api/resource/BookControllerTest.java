...
public class BookControllerTest {
    ...
    public void deleteButNotFoundBookTest() throws Exception {
        BDDMockito.given(service.getById(Mockito.anyLong()) ).willReturn(Optional.empty());
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders
                .delete(BOOK_API.concat("/"+1));
        mvc.perform(request)
    ...
    @Test
    @DisplayName("Deve atualizar um livro pela ID")
    public void updateBookTest() throws Exception {
        Long id = 1L;
        String json = new ObjectMapper().writeValueAsString(createNewBook());

        Book updatingBook = Book.builder().id(id).title("titulo2").author("autor2").isbn("3211").build();
        BDDMockito.given(service.getById(id) )
                .willReturn(Optional.of(updatingBook));
        Book updatedBook = Book.builder().id(id).title("As Aventuras").author("Artur").isbn("001").build();
    ...
    public void findBookTest() throws Exception {
        ...
        String querString = String.format("?title=%s&author=%s&page=0&size=100",book.getTitle(),book.getAuthor());
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get(BOOK_API.concat(querString))
                .accept(MediaType.APPLICATION_JSON);
        mvc.perform(request)
                .andExpect(status().isOk())
